...
{
    // Everything emitted below is written to compile at C# 7.3 — the default for
    // netstandard2.0 and net4x, which are exactly the targets the polyfill exists to
    // serve. So: no file-scoped namespaces, no primary constructors, no bodyless type
    // declarations, no `#nullable` and no nullable annotations. Modern consumers lose
    // nothing by it; older ones could not compile the file at all otherwise.
    //
    // The one feature with no C# 7.3 equivalent is `global using`, so this file is the
    // only output gated on the consumer's LanguageVersion.
    ...
    // C# 7.3 has no file-scoped namespaces. Wrapping at build time rather than in the
    // literals keeps them readable here and the emitted file correctly indented.
    static string InNamespace(string namespaceName, string body)
    {
    ...
    const string polyfillHeader = "// <auto-generated/>\n";
    ...
    // The constructor arguments are deliberately not stored: the analyzer reads them
    // from metadata via AttributeData.ConstructorArguments, never from an instance.
    const string typesBody =
        ...
        [DebuggerNonUserCode]
        sealed class UnionSyntaxAttribute : System.Attribute
        {
            public UnionSyntaxAttribute(params string[] options)
            {
            }
        }
        ...
        [ExcludeFromCodeCoverage]
        [DebuggerNonUserCode]
        sealed class ReturnSyntaxAttribute : System.Attribute
        {
            public ReturnSyntaxAttribute(params string[] syntax)
            {
            }
        }
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        // which is a far better outcome than the whole generated file failing to compile

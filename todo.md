...
- [ ] **1. Stack overflow on a self-referential local**
...
- [ ] **2. The package only loads on compiler 5.9 or newer**
...
- [ ] **4. `[UnionSyntax(null)]` and `[ReturnSyntax(null)]` crash the analyzer**
...
- [ ] **5. Annotations from a library with an internal `StringSyntaxAttribute` polyfill are ignored**
...
- [ ] **6. Nested lambdas bind the outer parameter to the inner collection**
...
- [ ] **7. Property setters get a false SSA002 with no fix**
  - Repro:
...
- [ ] **8. `//language=` comments are picked up too broadly**
...
- [ ] **9. Generic members get SSA002, and the fix annotates them for every `T`**
...
- [ ] **10. A local's `//language=` comment is never checked against its initializer**
  - Repro:
...
- [ ] **11. A reassigned local keeps its initializer's tag**
...
- [ ] **12. Adding an attribute detaches the member's doc comment**
...
- [ ] **13. The SSA008 fix deletes the trivia around the attribute**
...
- [ ] **14. SSA007 and SSA008 drop `StringSyntax` arguments**
...
- [ ] **15. Fixes land on the enclosing declaration when the fix site is inside an initializer**
  - Repro:
...
- [ ] **16. With `StringSyntaxAnalyzer_EmitGlobalUsings=false`, some fixes produce code that doesn't compile**
...
- [ ] **17. The delegate fix never clears the warning**
...
- [ ] **18. User-defined indexers get SSA002 with no fix**
...
- [ ] **19. The SSA005 docs don't match the code**
